...
**Companion analysis:** `wrk_docs/2025.12.23 - DOC - Feature Comparison vs Vanilla Minecraft.md`  
**Implementation journal:** `wrk_journals/2025.12.23 - JRN - Vanilla Parity Implementation.md`  
...
- **Fire block parity (partial):** a real `fire` block now exists (emissive + alpha-rendered) that can be placed by flint-and-steel, can be started by lightning and (simplified) lava ignition, is extinguished by water and sky-exposed precipitation, and ignites the player/mobs on contact (vanilla-ish; simplified).
