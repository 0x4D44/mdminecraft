...
- Uses a dedicated **alpha-blended pass** (fluids + select translucent blocks like ice) so translucency blends without hiding underlying geometry.
- Still lacks the full vanilla separation/ordering behavior for all translucent blocks (glass-like blocks may not match vanilla visuals).
...
1. **Rendering fidelity**
   - True translucent-block layer for glass/ice-like blocks (beyond cutout alpha-test).
