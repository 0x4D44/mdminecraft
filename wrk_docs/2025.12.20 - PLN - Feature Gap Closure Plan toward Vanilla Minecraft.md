...
**Implementation journal:** `wrk_journals/2025.12.20 - JRN - Vanilla Parity Implementation.md`  
...
- Commands:
  ...
  - Current limitations:
    ...
    - command-driven voxel edits perform unsupported-neighbor cleanup (e.g., torches/wires break when their support is removed), and crops/sugar cane placed via commands are registered for deterministic growth
    - `effect` is currently player-only (no target selectors); effect-name parsing is a small subset of vanilla ids
    - `clear` and `kill` are currently player-only (no target selectors); `/clear` supports a `maxCount` dry-run via `0`
- Content packs:
