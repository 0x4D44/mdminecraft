...
- **Stage 5B partial shipped:** chunk height increased to **384** (world range `0..383`) with world-space Y routed through `WORLD_MIN_Y/WORLD_MAX_Y` plus `world_y_to_local_y` / `local_y_to_world_y`; the negative-Y origin shift remains pending (see journal).
...
- Chunks are now stored internally as 16×16×16 vertical sections (exposed as a 16×384×16 `Chunk` API with world range `0..383`).
- World-space Y bounds/indexing is centralized behind `WORLD_MIN_Y/WORLD_MAX_Y` and conversion helpers so the later negative-Y expansion can shift origin in one place without rewriting callsites.
- Chunk persistence still writes a linear voxel stream sized by `CHUNK_VOLUME`; height expansion changed the expected voxel count, so a save migration story for older 256-height worlds remains pending.
...
- (Already done) Expand the vertical range to 384 blocks (current world range `0..383`).
- (Pending) Shift origin to allow negative Y coordinates (target: `-64..319`).
