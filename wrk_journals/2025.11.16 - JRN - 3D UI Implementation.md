...
**Raycast System:**
- [ ] Implement DDA (Digital Differential Analyzer) ray marching
- [ ] Cast ray from camera through mouse cursor
- [ ] Find first solid block intersection
- [ ] Calculate face normal of hit

**Block Highlighting:**
- [ ] Render wireframe outline around targeted block
- [ ] Show which block player is looking at
...
- [ ] Update chunk mesh
- [ ] Play break sound (placeholder for now)
...
Block interaction requires raycasting from the camera through the world to find which block the player is looking at. The DDA algorithm efficiently steps through voxel grid along a ray.

Steps:
1. Create raycast module with DDA implementation
2. Add block target tracking to App state
3. Render highlight box around targeted block
4. Handle mouse input to break blocks
5. Modify chunk data and regenerate mesh
