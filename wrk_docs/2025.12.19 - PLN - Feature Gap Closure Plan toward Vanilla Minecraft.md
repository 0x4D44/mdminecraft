...
**Date:** 2025-12-19  
**mdminecraft snapshot:** `92f40ccc54341e08878e456518c320b2b715684a`  
**Companion analysis:** `wrk_docs/2025.12.16 - DOC - Feature Comparison vs Vanilla Minecraft.md`  
**Implementation journal:** `wrk_journals/2025.12.19 - JRN - Vanilla Parity Implementation.md`  
...
- **Stage 4 in progress:** collisions + placement rules were upgraded for common non-cube blocks (stairs/slabs/trapdoors/doors/fences/panes/gates/walls), **slabs can stack into “double slabs”**, doors/trapdoors/gates now respond to **redstone power** (iron doors are usable), and custom meshing now includes neighbor-aware connectivity + stair corner shapes (inner/outer) for panes/fences/gates/walls/stairs (see journal for exact changes). Connectivity was tightened to avoid “connecting” to non-full blocks (e.g., doors/trapdoors), and panes/bars now connect to each other (vanilla-ish).
...
- **Stage 10 automation continued:** droppers/dispensers are now craftable/placeable with persisted inventories and dispense one item on power rising-edge; dispensers special-case buckets for water/lava placement/pickup; UIs are implemented (3×3 slots; shift-move + drag distribution) (see journal).
...
- **Test runtime hygiene:** heavy worldtests use debug-friendly defaults and env overrides; `determinism_worldtest` defaults are tuned to finish well under the 60s “looks hung” threshold (override via `MDM_DETERMINISM_CHUNK_RADIUS` / `MDM_DETERMINISM_VERIFICATION_ROUNDS` for heavier runs).
- **Test runtime hygiene:** long-running unit tests sample fewer chunks by default to avoid misleading “over 60 seconds” output in `cargo test` (see journal).
...
- Current deviations (explicitly accepted for now):
  ...
  - dispensers only implement bucket/fluid behavior so far (no projectile/block behaviors yet)
...
- Content packs + deterministic scripting: not started in this stage yet.
...
- Commands:
  ...
  - core set implemented: `tp`, `give`, `time`, `weather`, `gamemode`, `setblock`, `summon`
  ...
  - Current limitations:
    ...
    - `setblock` is currently limited to **loaded chunks** and uses default block state `0` (no blockstate args yet)
- Content packs:
