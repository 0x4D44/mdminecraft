...
use mdminecraft_testkit::{
    MetricsReportBuilder, MetricsSink, TestResult, TerrainMetrics, SeamValidation,
...
fn large_scale_terrain_worldtest() {
    ...
    for chunk_z in -CHUNK_RADIUS..=CHUNK_RADIUS {
        for chunk_x in -CHUNK_RADIUS..=CHUNK_RADIUS {
            ...
            let right_pos = ChunkPos { x: chunk_x + 1, z: chunk_z };
            ...
            let bottom_pos = ChunkPos { x: chunk_x, z: chunk_z + 1 };
    ...
    println!("  Valid: {} ({:.2}%)", seams_valid, (seams_valid as f64 / seams_checked as f64) * 100.0);
    ...
    println!("Average generation: {:.2}ms/chunk", avg_gen_time_us / 1000.0);
    println!("Seam validation: {}/{} ({:.2}%)",
        seams_valid, seams_checked,
        (seams_valid as f64 / seams_checked as f64) * 100.0);
    println!("Determinism: {}/{} chunks verified",
        verify_count - determinism_failures, verify_count);
    ...
    assert_eq!(seams_failed, 0, "All seams must be continuous (max diff: {} blocks)", MAX_SEAM_DIFF);
    assert_eq!(determinism_failures, 0, "Terrain must be deterministic");
    assert!(chunks_generated > 2500, "Should generate at least 2,500 chunks");
    assert!(unique_biomes >= 5, "Should have at least 5 different biomes");
    assert!(avg_gen_time_us < 30_000.0, "Average generation must be under 30ms");
    assert!(max_seam_diff <= MAX_SEAM_DIFF, "Max seam difference must be <= {} blocks", MAX_SEAM_DIFF);
}
