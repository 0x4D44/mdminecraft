...
**Date:** 2025-12-25  
**mdminecraft snapshot:** `d2574eba99ead17eb1ddd231b1c77a3ae23fa298` (+ local changes in working tree)  
**Companion analysis:** `wrk_docs/2025.12.25 - DOC - Feature Comparison vs Vanilla Minecraft.md`  
**Implementation journal:** `wrk_journals/2025.12.25 - JRN - Vanilla Parity Implementation.md`  
...
- **Stage 2 persistence v2 is in place:** `world.meta` + `world.state` persist global sim time/weather plus player/entities/block-entities, with deterministic iteration order; dropped-item metadata (durability/enchantments) is persisted; migration tooling exists (`mdminecraft-cli` `save-upgrade`).
...
- **Stage 3 survival/combat feel is closing gaps:** ranged/melee line-of-sight rules are enforced (no hits/shots through walls), skeleton archery + arrow damage/pickups are in place, explosions use distance falloff with deterministic shielding, armor protections are damage-kind aware, fall-like damage (fall/ender-pearl) uses a dedicated kind (no base armor reduction; `Protection`/`Feather Falling` apply), armor is now enchantable via the enchanting table UI, `Unbreaking`/`Mending` now affect armor durability, `Respiration` extends underwater breathing, `Aqua Affinity` removes underwater mining penalty, `Depth Strider` improves water movement speed, bow enchantments (`Power`/`Punch`/`Flame`/`Infinity`) apply to player-fired arrows, skeleton bow drops now spawn with deterministic durability + rare enchantments, zombies now have a small deterministic chance to drop iron ingots (vanilla-ish), burning pigs/cows now drop cooked meat (vanilla-ish), chickens now lay eggs periodically (vanilla-ish), eggs can now be thrown and may hatch into chickens (vanilla-ish), arrows can now activate buttons on impact (vanilla-ish), pressure plates now trigger from players/mobs (and oak plates from dropped items; vanilla-ish), and container break/spill drops now preserve durability + enchantments (see journal).
- **Stage 4 in progress:** collisions + placement rules were upgraded for common non-cube blocks (stairs/slabs/trapdoors/doors/fences/panes/gates/walls), **slabs can stack into “double slabs”**, doors/trapdoors/gates now respond to **redstone power** (iron doors are usable), and custom meshing now includes neighbor-aware connectivity + stair corner shapes (inner/outer) for panes/fences/gates/walls/stairs (see journal for exact changes). Connectivity was tightened to avoid “connecting” to non-full blocks (e.g., doors/trapdoors), and panes/bars now connect to each other (vanilla-ish). Waterlogging foundation is now in place (fluid sim + bucket/place/break + key survival checks like drowning/sugar cane/farmland hydration and bottle filling), and rendering now includes waterlogged water volumes plus an alpha-blended pass (fluids + ice + glass/glass panes) so translucency blends correctly without hiding underlying block geometry; the alpha pass now draws via a dedicated alpha index buffer (no full-mesh double draw) and is routed via a per-vertex alpha flag driven by the `mdm:render/translucent` block tag, with alpha-pass effects driven by per-vertex metadata (alpha-kind + flow direction) rather than hard-coded numeric block IDs in the shader. Flowing fluids now render with **vanilla-ish sloped top surfaces** (per-corner heights derived from neighbor levels) and avoid internal faces/vertical “steps” between same-type fluids; fluid shading now blends **two scrolling samples** (flow-direction aware) using deterministic sim-time, better approximating vanilla animated textures. Texture alpha is now respected for cutout-style non-fluid blocks via alpha testing (for true cutout blocks; glass/glass panes now use the alpha-blended pass). Portals now render as vanilla-ish surfaces/planes (End portal: flat at 12/16; Nether portal: thin axis-aligned planes) in the alpha pass with simple procedural effects, and the runtime texture atlas now uploads mipmaps (with padding bleed + linear mip filtering) to reduce distant shimmer and mip “popping”. Grass (top + side overlay) + foliage + water now receive biome tinting (chunk-sampled) via `render/tint/*` tags (grass/leaves) plus a per-chunk water tint uniform; birch + spruce leaves now use fixed foliage tints (vanilla-ish). Sky + fog are now environment-aware at a coarse level (dimension-colored Nether/End atmosphere; biome-tinted underwater fog using vanilla-ish `water_fog_color` defaults + lite underwater color grading), and the Overworld sky + distance fog are now biome-tinted (temperature-based, vanilla-ish), with a simple procedural night star field (suppressed during precipitation). Weather precipitation particles are now biome/dimension-aware (rain↔snow via biome temperature + elevation bias; suppressed outside the Overworld). Voxel lighting is now dimension-aware (Nether/End have no sun-driven contribution). Greedy cube faces now include per-vertex ambient occlusion (AO) + per-vertex smooth lighting (light interpolation) shading.
- **Stage 4 polish:** deserts no longer show precipitation particles (vanilla-ish), precipitation is suppressed under cover (no “rain indoors”), lighting attenuation is now data-driven via `light_opacity` (e.g., `*_leaves=1`, `water/ice=3`, `lava=15`, and waterlogged blocks attenuate like water), and lava now uses the `blocks/lava` texture (with a legacy duplicate entry renamed to `lava_legacy` to avoid name→id collisions).
- **Weather parity continued:** added `WeatherState::Thunderstorm` with deterministic Overworld lightning flashes (visual-only), darker overcast shading, and `/weather thunder` command support.
- **Lighting parity continued:** block-light emission is now data-driven via `light_emission` (0..=15) in `blocks.json` (content-pack friendly), and Nether/End portals now emit vanilla-ish block light.
...
- **Stage 5B shipped:** chunk height increased to **384** with world range `-64..319`; world-space Y is routed through `WORLD_MIN_Y/WORLD_MAX_Y` plus `world_y_to_local_y` / `local_y_to_world_y`, and rendering now applies the same Y origin via `ChunkUniform.chunk_offset.y = WORLD_MIN_Y` so voxel meshes align with simulation coordinates (see journal).
...
- **Stage 10 automation continued:** droppers/dispensers are now craftable/placeable with persisted inventories and dispense one item on power rising-edge; droppers/dispensers also attempt to insert into container inventories directly in front (chests/hoppers/droppers/dispensers) before dispensing/dropping (vanilla-ish); dispensers special-case buckets for water/lava placement/pickup and can also fill glass bottles from water (glass bottle → water bottle; vanilla-ish), and can also fire arrows / throw splash potions / throw eggs / launch ender pearls (limited projectile behaviors) plus flint-and-steel ignition; dispensers can also insert Eyes of Ender into empty End portal frames (vanilla-ish portal automation). UIs are implemented (3×3 slots; shift-move + drag distribution) (see journal).
...
- **Stage 4 weather polish continued:** thunderstorm lightning now applies basic gameplay effects (damage + ignition, and charging creepers with larger explosions), complementing the existing visual lightning flashes; charged creepers have a simple cyan tint in the mob renderer.
- **Stage 4 weather accumulation started:** in cold biomes, nearby water sources can freeze into ice (faster during precipitation) and snow can accumulate on exposed surfaces as **snow layers** (1..=8, vanilla-ish). Accumulation is **suppressed near strong block-light** (e.g., torches); snow/ice can **melt back** under strong block-light (torch-melt parity) and can also melt under strong **daylight skylight** in warm biomes (partial vanilla-ish); freezing now notifies the fluid simulator so adjacent flows react on the next tick.
- **Mob survival-rule parity:** zombies and skeletons now ignite in daylight when **sky-exposed** (column clear above head) and are extinguished by rain/snow (when exposed) and water (vanilla-ish).
- **Mob combat parity:** ranged projectile mobs now require **line of sight** (no shooting through walls), arrows now damage the player, and skeletons can fire arrows deterministically at range with gravity-aware aiming (basic vanilla-ish ranged pressure); hostile melee hits now also require line-of-sight (no hitting through walls); projectile hits, hostile melee hits, and creeper explosions now apply a vanilla-ish knockback to the player (collision-resolved), improving combat “feel” under ranged pressure.
- **Combat timing parity:** mobs now have brief post-hit invulnerability (“hurt cooldown”), reducing rapid multi-hit DPS and matching vanilla-ish feel (still simplified vs vanilla).
- **Armor enchant parity (partial):** `Protection` / `Fire Protection` / `Blast Protection` / `Projectile Protection` now apply as damage-type-specific reductions for player damage; fire/lava damage now respects armor reduction (still simplified vs vanilla, especially durability rules and other DOT).
- **Creeper parity:** creeper explosion damage now scales with distance and a deterministic exposure factor (wall shielding), and now also damages/knocks back nearby mobs (simplified), closer to vanilla expectations.
- **Ghast parity:** ghast fireballs now explode with distance-falloff + deterministic exposure (wall shielding) for AoE damage/knockback, so near-misses and block impacts are dangerous while solid cover protects (simplified vs vanilla).
- **Player combat parity:** player melee hits are now blocked by solid collision geometry (no hitting mobs through walls), using a collision-shape sweep along the attack ray to find the first blocking surface.
- **Bugfix parity:** mob fire damage now applies even when fire is continuously refreshed (e.g., daylight burning), via a deterministic burn timer.
- **Mob loot parity:** skeletons now drop arrows in addition to bones (deterministic count).
- **Mob loot parity:** skeletons now have a small deterministic chance to drop a bow.
- **Mob AI parity:** skeletons now use a simple deterministic ranged “keep distance + strafe” behavior (instead of zombie-like melee chasing), improving feel parity.
- **Archery loop parity:** arrows that hit blocks now drop arrow pickups (instead of persisting as stuck projectiles), enabling vanilla-ish arrow recovery; projectile block collision now respects thin/tall collision shapes (slabs/snow layers, open doors/trapdoors/gates, fences/walls) and uses a per-tick sweep to avoid tunneling through thin geometry; projectile entity collisions (mobs + player) now use a swept segment-vs-AABB test so fast projectiles can’t skip hitboxes, and entity hits resolve before later block impacts within the same tick.
- **Item durability parity (improved):** bows are now non-stackable and consume durability on shot (deterministic `Unbreaking`), and flint-and-steel consumes durability when igniting new fires/portals; **durability and enchantments are now preserved when items are dropped and picked up** (player drop/pickup + dispenser/dropper drops + hopper pickup). Some vanilla behaviors remain simplified (e.g., mob tool drops and repair rules; exact enchantment generation/rarity and many edge-case interactions).
- **Fire parity:** rain/snow and water now extinguish burning mobs and the player when sky-exposed (deserts excluded), matching vanilla-ish expectations.
- **Fire block parity (partial):** a real `fire` block now exists (emissive + alpha-rendered) that can be placed by flint-and-steel, can be started by lightning and (simplified) lava ignition, is extinguished by water and sky-exposed precipitation, ignites the player/mobs on contact, and now has a deterministic age/decay/spread tick with simple flammable-support burning (vanilla-ish; simplified). Flammability coverage now includes common logs/leaves and wood derivatives (not just oak log/planks).
- **Content/pack compatibility:** legacy `emissive: true` is accepted as a fallback for `light_emission` (treated as level 15 when `light_emission` is omitted) to keep older configs/packs working; stateful emitters (redstone torch, respawn anchor) still override emission even when they evaluate to `0`.
- **Test runtime hygiene:** heavy worldtests use debug-friendly defaults and env overrides; `determinism_worldtest` defaults are tuned to finish well under the 60s “looks hung” threshold (debug defaults now generate a single chunk; override via `MDM_DETERMINISM_CHUNK_RADIUS` / `MDM_DETERMINISM_VERIFICATION_ROUNDS` for heavier runs), `persistence_roundtrip_worldtest` defaults are tuned similarly (override via `MDM_PERSISTENCE_ROUNDTRIP_CHUNK_RADIUS`), `large_scale_terrain_worldtest` defaults are tuned similarly (override via `MDM_LARGE_SCALE_TERRAIN_CHUNK_RADIUS`), `biome_seams_worldtest` defaults are tuned similarly (override via `MDM_BIOME_SEAMS_CHUNK_RADIUS` and `MDM_BIOME_SEAMS_MAX_AVG_GEN_US`), and `stage4_integration_worldtest` defaults are tuned similarly (override via `MDM_STAGE4_INTEGRATION_CHUNK_RADIUS` and `MDM_STAGE4_INTEGRATION_MAX_AVG_GEN_US`).
...
- Droppers/dispensers insert into inventories directly in front (chest/hopper/dispenser/dropper) before their normal behaviors (vanilla-ish) (**done**).
- Dispensers can fill glass bottles from water (glass bottle → water bottle; vanilla-ish) (**done**).
...
- Current deviations (explicitly accepted for now):
  ...
  - dispensers still lack most vanilla dispense behaviors (block placement, armor equipping, etc.); only buckets, a small set of projectile items, glass-bottle filling, flint-and-steel ignition, Eye-of-Ender insertion, and container insertion are implemented.
