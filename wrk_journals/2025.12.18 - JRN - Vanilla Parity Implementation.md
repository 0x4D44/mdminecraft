...
### Stage 4 — Basic support checks (WIP)

- Added deterministic support checks for ladders and torches:
  - Placement: ladders/torches now require a solid supporting block.
  - Updates: when a supporting block is removed (mining or explosions), adjacent ladders/torches are removed deterministically via `GameWorld::remove_unsupported_blocks`, lighting is recomputed for affected chunks, and meshes are refreshed.
- Wired creeper-explosion block destruction to notify fluid simulation of removed blocks (plus any support-driven removals) so water/lava can respond immediately.
- Added unit tests to lock in torch/ladder support removal behavior.

### Verification (post-change 51)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Redstone floor-attachment checks (WIP)

- Restricted placement of levers/buttons/pressure plates/redstone wire to top faces (floor attachment only for now), requiring a solid supporting block.
- Extended `GameWorld::remove_unsupported_blocks` to remove these components deterministically when their supporting block is removed; added a unit test for redstone wire support removal.

### Verification (post-change 52)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Dropped items: ladders + redstone components (WIP)

- Extended `mdminecraft_world::drop_item::ItemType` and placement/drop mappings to cover:
  - `ladder`, `lever`, `stone_button`, `oak_button`, `stone_pressure_plate`, `oak_pressure_plate`, `redstone_wire`, `redstone_torch`
- These blocks now:
  - drop when mined (via `ItemType::from_block`)
  - can be dropped from the inventory UI (core block → dropped item mapping)
  - can be picked up back into core inventory (dropped item → core block mapping)

### Verification (post-change 53)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Drops + door support integrity (WIP)

- Expanded dropped-item coverage for common building/utility blocks:
  - glass panes
  - fences / fence gates
  - slabs / stairs
  - trapdoors / chests
  - doors
  - redstone lamps
  - enchanting tables / brewing stands
  - nether wart blocks / soul sand
  - obsidian
  - birch/pine logs
- Made `ice` and `glass` Silk Touch-only drops; enabled Silk Touch to drop oak/birch/pine leaves and made leaf blocks placeable as block items.
- Tightened door placement: only lower halves; top-face placement only.
- Extended `GameWorld::remove_unsupported_blocks` to remove doors when support below is removed or when halves are missing (includes downward neighbor checks); added a unit test for door support removal.
- Updated `drop_item_lifecycle_worldtest` to reflect ice being Silk Touch-only.

### Verification (post-change 54)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Wall-mounted torches (WIP)

- Added a compact, non-conflicting block-state encoding for wall-mounted torches (keeps low bits free for redstone power/active state).
- Torch placement now supports:
  - floor torches (top-face placement)
  - wall torches (side-face placement)
  - (still disallows ceiling-mounted torches)
- Support updates now remove wall torches when their supporting wall block is removed.
- Updated torch rendering to offset wall torches toward their supporting wall.
- Updated redstone torch inversion logic to check the supporting block when wall-mounted (instead of always checking the block below).
- Added unit tests for torch wall placement state + support removal.

### Verification (post-change 55)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Wall/ceiling-mounted levers and buttons (WIP)

- Generalized the torch wall-mount encoding into reusable “mount” state helpers (`wall_mount_state`, `ceiling_mount_state`, `is_wall_mounted`, `is_ceiling_mounted`) while keeping the existing torch helpers as wrappers.
- Lever and button placement now supports floor, wall, and ceiling mounts (pressure plates + redstone wire remain top-face only).
- Support updates now remove wall/ceiling levers and buttons when their supporting block is removed.
- Updated lever/button rendering to respect mount state:
  - buttons render on floor/wall/ceiling with pressed depth adjustment
  - levers render on floor/wall/ceiling with a simplified protruding handle and active-state offset
- Added unit tests for button placement state and lever/button support removal (wall + ceiling cases).

### Verification (post-change 56)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Redstone + interactions wired into gameplay (WIP)

- Integrated `RedstoneSimulator` into the main game loop (`fixed_update` at 20 TPS), including mesh refresh for dirty chunks and their neighbors.
- Added player pressure-plate triggering (tracks the plate under the player each tick and updates redstone state deterministically).
- Wired right-click interactions in gameplay:
  - doors / trapdoors / fence gates now toggle instead of “placing a block” (via `InteractionManager`)
  - levers toggle and buttons press (via `RedstoneSimulator`)
- Scheduled redstone updates around block placement/break events so newly placed/removed components cause networks (wire/torch/lamp) to settle without manual nudges.

### Verification (post-change 57)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Redstone updates after explosions (WIP)

- Creeper explosion block destruction now schedules redstone updates around removed blocks (and any support-driven removals) so nearby circuits settle naturally after an explosion.

### Verification (post-change 58)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Interaction precedence + test runtime hygiene (WIP)

- Right-click now prioritizes **block interactions** (doors/trapdoors/fence gates/levers/buttons + key UIs) before item “use” actions like eating/drinking/equipping, matching vanilla expectations more closely.
- Reduced default debug chunk radius for Stage 4 worldtests to avoid >60s “looks hung” warnings:
  - `stage4_integration_worldtest`: default radius 3 (was 4)
  - `stage4_metrics_worldtest`: default radius 3 (was 4)

### Verification (post-change 59)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 4 — Blocklight propagation integrated (WIP)

- Added a small, centralized block light emission table (vanilla-ish levels) and exposed it as `block_light_emission`.
- Fixed lighting seam stitching to seed from:
  - any **non-opaque** voxel for skylight
  - any voxel with **block light > 0** for blocklight (including opaque emissive blocks like lamps)
- Added `recompute_block_light_local` to clear/reseed local blocklight and stitch seams deterministically.
- Integrated blocklight recomputation into gameplay:
  - chunk load initializes blocklight
  - block placement/mining/explosions update blocklight and refresh affected meshes
  - fluids/redstone now track `dirty_light_chunks` so blocklight recompute runs only when lighting-affecting changes occur (lava/stone/obsidian changes; lamp/torch toggles)

### Verification (post-change 60)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Chest container UI + persisted inventories (WIP)

- Added a persisted chest block-entity inventory (`ChestState`, 27 slots) and wired it into `BlockEntitiesState` so it roundtrips through save/load.
- Implemented a functional chest UI:
  - right-click chest opens UI
  - shift-click moves items between chest and player storage
  - drag distribution works across chest + player inventory slots
  - `E` / `Escape` closes the chest (and other open UIs) without opening overlapping overlays
- Added block-entity cleanup on block removal:
  - mining, explosions, and support-driven removals now remove block-entity state for chests/furnaces/brewing stands/enchanting tables
  - dropped contents on removal (where representable by `DroppedItemType`) to avoid “ghost” inventories and furnace re-lighting side effects after breakage
- Added unit coverage for chest shift-move behavior.

### Verification (post-change 61)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Chests are craftable (WIP)

- Added a chest crafting recipe (8 planks → chest) to the existing crafting + recipe-book system so players can obtain chests in survival.
- Added unit coverage for the chest recipe.

### Verification (post-change 62)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Shaped crafting + building/redstone recipes (WIP)

- Added shaped crafting support:
  - `CraftingPattern` (3×3 cell pattern + width/height) and `CraftingRecipe::pattern`.
  - Shaped recipe matching runs before shapeless matching and supports offset placement within the active grid size.
- Updated “Fill” (recipe-book-lite) to autofill shaped patterns and consume the correct materials.
- Fixed craft-all output accounting so shift-craft only produces what was actually consumed.
- Added/updated recipes to unlock common vanilla-ish early building + redstone:
  - sticks (vertical), ladder, oak door, trapdoor, oak fence + gate
  - oak/stone buttons, lever, oak/stone pressure plates
  - slabs + stairs (including mirrored stair patterns)
- Added unit coverage for the new shaped recipes and symmetry cases.

### Verification (post-change 63)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Shaped crafting parity cleanup (WIP)

- Converted several previously “count-only” recipes to shaped patterns to better match vanilla expectations:
  - furnace + chest now require the ring pattern (empty center)
  - torches now require coal above stick
  - bow + arrow now require their shaped patterns (bow supports mirrored orientation)
- Added a `CraftingRecipe::allow_horizontal_mirror` flag so recipes like stairs/bow accept mirrored placement without duplicating recipe-book entries.
- Updated autofill + unit tests to reflect shaped-slot filling (1 item per required cell).

### Verification (post-change 64)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — More shaped recipes (WIP)

- Crafting table is now a true 2×2 shaped recipe (works with offsets in the 3×3 table like vanilla).
- Armor crafting moved from “count-only” to shaped patterns for leather/iron/diamond sets (helmet/chestplate/leggings/boots).
- Added unit coverage for shaped bow + arrow (and mirrored bow orientation).

### Verification (post-change 65)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Tool crafting recipes (WIP)

- Added full vanilla-ish shaped recipes for tool crafting across all tool materials (wood/stone/iron/diamond/gold):
  - pickaxe, axe (mirrorable), shovel, sword, hoe (mirrorable)
- Integrated into the existing crafting UI + recipe-book-lite list.
- Added unit coverage for wooden pickaxe + mirrored axe/hoe patterns.

### Verification (post-change 66)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Beds are craftable + multi-block placement/removal (WIP)

- Added a shaped bed recipe (3 wool over 3 planks → bed).
- Implemented correct bed placement as a 2-block structure:
  - only `bed_foot` is placeable; placement spawns the matching `bed_head` based on facing
  - placement checks for air in both positions and solid support below both halves
  - chunk lighting/mesh refresh now accounts for cross-chunk multi-block placements (beds can straddle chunk borders)
- Implemented correct bed break/removal pairing:
  - mining either half removes the other half (including cross-chunk cases)
  - support-driven removals now remove beds when either half loses support or when the other half is missing
- Added `DroppedItemType::Bed` so beds drop/pick up consistently, mapping both halves to the same item.
- Added unit coverage for:
  - bed crafting
  - bed support removal behavior
  - drop-item mappings for bed IDs.

### Verification (post-change 67)
...
### Stage 3 — Bed cross-chunk correctness (WIP)

- Fixed a cross-chunk edge case: mining one half of a bed that straddles chunk boundaries now updates lighting + meshes for both chunks.
- Added a unit regression test to ensure the bed “paired half” removal works across chunk boundaries.

### Verification (post-change 68)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Diamonds are obtainable (WIP)

- Diamond ore now drops diamonds (resource item) instead of dropping the ore block, enabling survival progression to diamond tools/armor.
  - Silk Touch still yields the ore block (unchanged).
  - Fortune now correctly increases diamond drop counts (via the existing fortune path).
- Fixed dropped↔core item conversions so diamonds can be picked up, carried, and dropped/spilled consistently.
- Updated/added unit + worldtest coverage to lock in the new mapping and conversions.

### Verification (post-change 69)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Glass progression: smelting + panes (WIP)

- Added sand → glass smelting support in the furnace recipes.
- Added a shaped glass panes recipe (6 glass → 16 panes) to the crafting table recipe set.
- Added unit coverage for both the smelt output mapping and the panes crafting recipe.

### Verification (post-change 70)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Iron doors craftable + worldtest perf guardrail tweak (WIP)

- Added an iron door crafting recipe (6 iron ingots → 3 iron doors).
- Added unit coverage for the iron door recipe.
- Reduced flakiness in `biome_seams_worldtest` by:
  - loosening the default debug-mode perf budget, and
  - allowing an override via `MDM_BIOME_SEAMS_MAX_AVG_GEN_US` (microseconds).

### Verification (post-change 71)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Brewing + enchanting become survival-reachable (WIP)

- Added missing **early brewing chain**:
  - Shaped recipe: `3 glass → 3 glass bottles`.
  - Right-click **water with a glass bottle** to fill a water bottle (vanilla-ish: you hold the filled bottle; remaining empty bottles go to inventory or spill).
  - Shapeless recipe: `1 nether wart block → 9 nether wart` (vanilla behavior).
  - Shaped recipe: `1 blaze powder + 3 cobblestone → brewing stand` (blaze rod substitute).
- Added a **vanilla-ish enchanting table recipe** (book substitute):
  - `4 obsidian + 2 diamonds + 1 lapis → enchanting table`.
- Made key progression resources **obtainable in Overworld**:
  - Worldgen now spawns **lapis ore** (y≤32).
  - Worldgen now spawns small amounts of **nether wart blocks + soul sand** (y≤32) as Overworld proxies for brewing progression.
  - **Magma blocks** now drop **blaze powder** (Overworld proxy for Nether blaze powder).
- Added unit coverage for:
  - glass bottle crafting
  - nether wart block → nether wart crafting
  - brewing stand crafting
  - enchanting table crafting
  - lapis/nether-proxy ore generation, and magma block drop mapping

### Verification (post-change 72)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 0/3 — Brewing determinism snapshot + enchanting bookshelf fix (WIP)

- Added a deterministic micro-worldtest snapshot that covers:
  - water bottles + nether wart → awkward potions
  - awkward + blaze powder → strength potions
  - golden file: `crates/world/tests/snapshots/micro_brewing_water_to_awkward_to_strength.json`
- Fixed enchanting table bookshelf counting:
  - previously used `47` (farmland) as a “bookshelf” placeholder; now counts **0** until a real bookshelf block exists in `config/blocks.json`.
- Reduced CI/local flakiness in `large_scale_terrain_worldtest` by:
  - loosening the default debug perf budget, and
  - allowing an override via `MDM_LARGE_SCALE_TERRAIN_MAX_AVG_GEN_US` (microseconds).

### Verification (post-change 73)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Creeper drops: gunpowder is now pick-up-able (WIP)

- Fixed a survival progression bug: creepers could drop gunpowder, but the client inventory had no dropped↔core mapping so it couldn’t be picked up.
  - Added `CORE_ITEM_GUNPOWDER` and conversion mapping in both directions.
  - Added unit coverage via the existing conversion test.

### Test runtime hygiene (follow-up)

- Added env overrides + loosened debug perf budgets for Stage 4 worldtests to reduce flakiness on slower runners:
  - `MDM_STAGE4_INTEGRATION_MAX_AVG_GEN_US`
  - `MDM_STAGE4_METRICS_MAX_AVG_GEN_US`

### Verification (post-change 74)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Basic farming: wheat → bread (WIP)

- Added dropped-item support for **wheat seeds**, **wheat**, and **bread** (stable IDs appended), plus dropped↔core conversion mappings so pickup/drop works end-to-end.
- Added hoe interaction: right-click dirt/grass with a hoe to till farmland (requires air above; consumes durability).
- Added seed interaction: right-click farmland with wheat seeds to plant wheat (registers crop for deterministic growth).
- Integrated `CropGrowthSystem` into the 20 TPS sim loop and chunk load/unload:
  - crops tick deterministically (growth + farmland hydration),
  - crop positions register on chunk load and are removed on unload to avoid unbounded growth state.
- Added vanilla-ish drops:
  - breaking grass has a deterministic 1/8 chance to drop wheat seeds,
  - breaking mature wheat drops extra seeds in addition to wheat.
- Added a bread recipe: `3 wheat → bread` (3×3 grid), plus unit coverage.
- Made farmland hydration “pay for play” (skips chunk scans when no crops are registered) and updated hydration tests accordingly.

### Verification (post-change 75)

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace`

### Stage 3 — Bookshelves enable stronger enchanting (WIP)

- Added a **bookshelf block** to `config/blocks.json` (appended to preserve existing block IDs).
- Wired enchanting bookshelf counting to the real bookshelf block (instead of always returning 0).
- Added a **vanilla-ish bookshelf recipe**: `6 planks + 3 wheat → bookshelf` (books substitute).
- Added dropped-item support so breaking/placing bookshelves works like other placeable blocks.
- Added unit coverage for the recipe and block-id consistency.

### Verification (post-change 76)
