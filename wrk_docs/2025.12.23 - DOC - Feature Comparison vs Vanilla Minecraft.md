...
- Runtime atlas upload generates **mipmaps** (alpha-aware, with padding bleed) so distant blocks shimmer less (more vanilla-ish).
...
- Sky + fog are now environment-aware (dimension-colored Nether/End atmosphere; biome-tinted underwater fog), and the Overworld sky is now biome-tinted (temperature-based, vanilla-ish), but exact vanilla per-biome sky/fog/water-fog rules remain simplified.
...
- Water surface tint is now biome-driven (chunk-sampled), but underwater fog/tint and vanilla’s exact colormap behavior are still simplified.
...
1. **Rendering fidelity**
   ...
   - Fluid flow-direction texturing/overlays beyond the current coarse directional cue.
2. **Content breadth**
