...
- (Planned) Update skylight propagation and any remaining lighting edge cases across sections.
- (Planned) Consider a persistence region format v2 (sectioned payloads) once the vertical model stabilizes.
- (Planned) Consider section-addressed network chunk streaming payloads if/when bandwidth becomes limiting.
