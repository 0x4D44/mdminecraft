...
- Configured blocks: `config/blocks.json` currently defines `140` blocks (vanilla has hundreds+).
...
- Enchanting exists (bookshelf count + lapis), and can now enchant tools/bows/armor, but the offer/level formulas and enchantment catalog remain simplified vs vanilla.
...
- Skeletons now have a deterministic ranged attack (arrows) with gravity-aware aiming, and arrows can now damage the player; skeleton AI now uses a simple ranged “keep distance + strafe” behavior, but aiming/LOS/difficulty scaling are still simplified vs vanilla.
- Hostile ranged projectile firing now requires line-of-sight (no shooting through walls), using block collision shapes.
- Hostile melee damage now requires line-of-sight (no hitting through walls), using collision shapes.
- Creeper explosion damage now scales with distance and a deterministic exposure factor (wall shielding), and creeper explosions now also damage/knock back nearby mobs (still simplified vs vanilla).
- Ghast fireballs now apply explosion AoE damage/knockback with distance falloff and deterministic exposure (wall shielding), so near-misses and block impacts can hurt while solid cover protects (still simplified vs vanilla).
- Skeleton loot now includes arrows (in addition to bones), improving survival loop parity for archery.
- Skeletons now have a small deterministic chance to drop a bow, and dropped bows now spawn with deterministic durability and rare enchantments (still simplified vs vanilla’s difficulty/local-difficulty rules).
- Zombies now have a small deterministic chance to drop iron ingots (in addition to carrots/potatoes).
- Pigs and cows killed while burning now drop cooked meat (vanilla-ish).
- Chickens now lay eggs periodically (deterministic, vanilla-ish interval).
- Eggs can now be thrown as a projectile and may hatch 1 or 4 chickens (deterministic, vanilla-ish rates).
- Dispensers now throw eggs as egg projectiles (vanilla-ish).
- Eggs and eyes of ender now stack to 16 (vanilla-ish).
- Arrows that hit blocks now drop arrow pickups (vanilla-ish recovery loop), and projectile collision now respects thin/tall block collision shapes (slabs/snow layers, open doors/trapdoors/gates, fences/walls) with a per-tick sweep; projectile entity collision now also uses a swept segment-vs-AABB test (mobs + player), so fast projectiles can’t tunnel through hitboxes and entity hits win over later block impacts in the same tick; projectile hits (plus hostile melee hits / creeper explosions) now apply a vanilla-ish knockback to the player (still simplified vs vanilla).
- Bow enchanting is now supported and player-fired arrows can carry `Power`/`Punch`/`Flame`/`Infinity` effects; `Infinity` requires at least one arrow in inventory, does not consume arrows, and does not drop arrow pickups (simplified vs vanilla tuning).
- Bows are now non-stackable and consume durability when firing (vanilla-ish), and durable/enchanted items now preserve durability and enchantments when dropped and picked up (player drop/pickup + dispenser/dropper drops + hopper pickup + container break/spill drops).
- Player melee attacks now respect solid collision geometry (no attacking mobs through walls), using a collision-shape sweep along the attack ray.
- Mobs now get a brief invulnerability window after taking damage (“hurt cooldown”), matching vanilla-ish combat pacing (still simplified vs vanilla).
- Armor enchantment protection types now reduce their matching damage kinds (fire/blast/projectile) for player damage; fire/lava damage now respects armor reduction (still simplified vs vanilla, especially durability rules and other DOT).
- Lightning strike damage to the player now respects armor reduction (generic damage kind). Fall-like damage (fall and ender-pearl self-damage) uses a fall damage kind that bypasses base armor reduction but allows `Protection` and `Feather Falling` (boots) to apply (still simplified vs vanilla).
- Armor durability is still simplified vs vanilla, but `Unbreaking` now reduces durability loss on hits and `Mending` can now repair equipped armor from XP orbs.
- Respiration (helmet) is implemented as a vanilla-ish max-air increase (simplified vs vanilla’s probabilistic air-consumption rule).
- Aqua Affinity (helmet) removes the underwater mining speed penalty, and Depth Strider (boots) improves underwater movement speed (simplified vs vanilla tuning).
- Mob burning now deals damage reliably even when fire is refreshed each tick (needed for undead daylight burn parity).
- Rain/snow now extinguishes burning entities (player + mobs) when sky-exposed (biome-aware; deserts excluded).
- A real `fire` block now exists and ignites entities on contact; it can be created via flint-and-steel, lightning strikes, and (simplified) lava ignition. Fire now has a deterministic age/decay/spread tick and can burn away simple flammable supports; flammability coverage includes common logs/leaves and wood derivatives, but vanilla’s full fire rules (age/spread chance tuning, block burn lists, soul fire, and many edge interactions) remain simplified/unimplemented.
...
- Dispensers now handle some vanilla-ish special cases (arrows/splash potions/eggs as projectiles; bucket interactions; glass bottle filling from water; flint-and-steel ignition), but many other dispense behaviors are still missing.
- Droppers/dispensers now insert into container inventories directly in front (chests/hoppers/droppers/dispensers) before dispensing/dropping (vanilla-ish), enabling basic item routing/transfer contraptions.
- Buttons can now be activated by arrow impacts (vanilla-ish projectile interaction), but other projectile-triggered redstone interactions remain incomplete.
- Pressure plates now trigger from players/mobs, and oak plates trigger from dropped items (vanilla-ish), but full entity-shape rules and other plate variants are not implemented.
- Dispensers can now launch ender pearls as projectiles; only player-owned ender pearls teleport the player (vanilla-ish “no owner ⇒ no teleport”), but full ownership attribution (multiplayer) is not implemented.
- Dispensers can now insert Eyes of Ender into empty End portal frames (vanilla-ish), enabling automated portal completion.
...
- Dropped item entities persist durability/enchantments across save/load (engine-native v2 world state).
...
- Thunderstorms exist (`WeatherState::Thunderstorm`) with deterministic Overworld lightning flashes and darker overcast shading; lightning strikes now apply basic gameplay effects (damage + ignition, charging creepers, and starting small block fires via the `fire` block), but many vanilla interactions (lightning rods, villager conversions, etc.) are still not implemented.
