...
### Stage 5B groundwork — World-Y range plumbing

- Refactored world-space Y handling to go through `WORLD_MIN_Y/WORLD_MAX_Y` and conversion helpers:
  - Added `mdminecraft_world::{WORLD_MIN_Y,WORLD_MAX_Y,world_y_to_local_y,local_y_to_world_y}` and updated callsites to avoid `y as usize` casts that will break once negative Y is enabled.
  - Updated redstone/fluid/automation world-position indexing to use `world_y_to_local_y`.
  - Updated structure placement helpers and portal placement/activation helpers to validate/map Y via the shared helpers.
  - Updated meshing so neighbor sampling uses world-space `y` (`local_y_to_world_y`) and all voxel samplers map back to local indices (`world_y_to_local_y`).
  - Updated game-layer chunk sampling, collision neighbor queries, bed placement, and command voxel-edit paths to use the same helpers.
- Behavior is unchanged today because `WORLD_MIN_Y` remains `0`, but shifting the vertical origin is now largely centralized.

### Verification

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `crates/world/tests/determinism_worldtest.rs` finished in 22.94s)

### Stage 5A — Networking: centralize chunk volume constants

- Added chunk dimension constants (`CHUNK_SIZE_X/Y/Z`, `CHUNK_VOLUME`) to `mdminecraft-net` protocol definitions.
- Updated chunk streaming/encoding to use `CHUNK_VOLUME` (removes hard-coded `65536` assumptions and makes later height/protocol changes more localized).
- Updated net unit tests to size buffers based on `CHUNK_VOLUME` and made decompression-bomb tests scale with `MAX_DECOMPRESSED_SIZE`.

### Verification

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `determinism_worldtest` finished in 10.91s)

### Test runtime hygiene — persistence fuzz proptests

- Added a debug-friendly proptest case budget to `crates/world/tests/proptest_persist_fuzz.rs` to avoid spurious “running for over 60 seconds” output on slower machines.
  - Defaults: 32 cases in debug, 256 in release.
  - Override via `MDM_PROPTEST_PERSIST_CASES` when you want heavier coverage locally/CI.

### Verification

- `cargo fmt --all -- --check`
- `cargo test --workspace` (notably `proptest_persist_fuzz` finished in 4.63s)

### Stage 5A — Chunk vertical sections (no height change yet)

- Switched chunk voxel storage to **16×16×16 vertical sections** while keeping the external chunk API and world height at 256.
- Added `Chunk::linear_voxels()` (serde wrapper) so persistence can serialize section-backed chunks into the **legacy linear voxel stream** (no save format bump yet).
- Updated chunk persistence deserialization indexing to use `CHUNK_SIZE_X/CHUNK_SIZE_Z` constants (keeps ordering explicit).

### Verification

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `determinism_worldtest` finished in 10.84s)

### Stage 8 — Nether: fortress chest loot table

- Added `WorldgenChestLootTable::NetherFortress` and selection logic for Nether chests based on `mdminecraft_world::nether_fortress_contains(...)`.
- Implemented a vanilla-ish default loot distribution (nether wart, quartz, ingots, blaze powder, glowstone dust, diamonds, obsidian) with deterministic seeding and content-pack overrides (`table: "nether_fortress"` / `"fortress"`).
- Added unit coverage that locates the generated fortress chest deterministically and asserts table selection + override behavior.

### Test runtime hygiene

- Reduced debug default workload for `large_scale_terrain_worldtest` (chunk radius 4 → 3) to reduce “looks hung” output in `cargo test`.
- Relaxed the default debug performance budget in `biome_seams_worldtest` (avg chunk gen threshold 1.2s → 1.5s) to reduce flakiness on slower runners; still overridable via `MDM_BIOME_SEAMS_MAX_AVG_GEN_US`.

### Verification

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `mdminecraft-world` `determinism_worldtest` finished in 19.75s; `large_scale_terrain_worldtest` finished in 39.90s)

## 2025-12-21

### Stage 9 — Respawn anchors (Nether/End respawn)

- Added new blocks with stable IDs:
  - `glowstone` (light source; breaks into glowstone dust unless Silk Touch)
  - `crying_obsidian` (light source; vanilla-ish)
  - `respawn_anchor` (charge-based light emission)
- Added crafting:
  - 4× glowstone dust → 1× glowstone
  - 1× obsidian + 1× ghast tear → 1× crying obsidian (mdminecraft proxy acquisition)
  - respawn anchor recipe (crying obsidian + glowstone; vanilla-ish pattern)
- Respawn anchor gameplay:
  - charge with glowstone dust or glowstone (0–4) in the Nether/End
  - use (no glowstone) to set the player respawn point in the Nether/End
  - using a respawn anchor in the Overworld explodes (vanilla-ish)
- Respawn logic updated:
  - respawn now respects `spawn_point_dimension`
  - Nether/End respawns require a charged respawn anchor and consume one charge on respawn; missing/uncharged anchors fall back to Overworld spawn near origin
  - respawn now prefers a safe spawn near the stored `(x,y,z)` to avoid “spawn on roof” regressions

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 16.95s)

---

## 2025-12-21

### Stage 8 — Nether fortress-lite + hostiles (Blaze, Ghast)

- Added deterministic fortress-lite corridor generation in the Nether (stone bricks shell + chest), plus deterministic blaze spawn points tied to fortress placement.
- Implemented Blaze + Ghast flying AI and deterministic fireball projectiles (`ProjectileType::{BlazeFireball,GhastFireball}`) with game-layer collision/damage handling.
- Fireball polish:
  - Blaze/Ghast fireballs now ignite struck mobs/players (respects Fire Resistance).
  - Ghast fireballs trigger a small explosion that can destroy blocks (vanilla-ish; simplified).
- Added deterministic firing-schedule unit coverage for blaze/ghast fireballs in `mdminecraft-world`.
- Added rare chunk-generation ghast spawns in the Nether so exploration surfaces hazards without additional periodic spawn rules.
- Added a simple periodic Nether ghast spawn rule (low cap) and fixed “daytime despawn” to apply only in the Overworld.
- Added integration test `crates/world/tests/nether_fortress_blaze_spawns.rs` validating blaze spawn points align with the generated corridor.

### Test runtime hygiene

- Reduced default debug workload of `crates/world/tests/persistence_roundtrip_worldtest.rs` (chunk radius 2→1) to avoid the 60s “running for over 60 seconds” warning; heavier runs can override via `MDM_PERSISTENCE_ROUNDTRIP_CHUNK_RADIUS`.
- Reduced default debug workload of `crates/world/tests/biome_seams_worldtest.rs` (chunk radius 4→1) to avoid the same warning; heavier runs can override via `MDM_BIOME_SEAMS_CHUNK_RADIUS`.
...
### Verification (post test-hygiene tweak)

- `cargo fmt --all`
- `cargo clippy -p mdminecraft-world --tests -- -D warnings`
- `cargo test -p mdminecraft-world --test persistence_roundtrip_worldtest -- --nocapture` (34.86s)

### Stage 9 — End: groundwork (worldgen + environment rules)

- Added `end_stone`:
  - appended to `config/blocks.json` (stable base registry ID)
  - stone-like block properties + dropped-item mapping so it behaves like other terrain blocks
- Updated End terrain generation:
  - `DimensionId::End` now uses a deterministic central-island profile near the origin and void elsewhere
  - a bedrock floor is intentionally retained to preserve engine invariants (deviation from vanilla’s void)
- Added vanilla-ish dimension rules:
  - water buckets evaporate immediately in the Nether (bucket becomes empty; no water is placed)
  - beds explode when used in the Nether/End (damage + block destruction; explosion SFX)

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `crates/world/tests/determinism_worldtest.rs` finished in 11.11s)

### Stage 9 — End: portal entry path (End Portal)

- Added End portal blocks:
  - `end_portal_frame` and `end_portal` appended to `config/blocks.json` with stable IDs and block properties.
- Added ender items:
  - `ender_pearl` (stacks to 16) and `eye_of_ender` core items with dropped-item mappings.
  - `/give minecraft:ender_pearl` and `/give minecraft:eye_of_ender` now work (vanilla-ish parsing).
- Implemented portal activation:
  - Using an Eye of Ender on an End Portal Frame (Overworld only) inserts an “eye” bit (stored in `Voxel.state`).
  - A filled 5×5 frame ring activates a 3×3 End portal interior (fills `end_portal` blocks).
- Implemented portal travel:
  - Standing in End portal blocks triggers Overworld↔End travel; End → Overworld returns near the player spawn point.
- Added acquisition hooks (mdminecraft alternative progression for now):
  - Shapeless crafting: `ender_pearl + blaze_powder → eye_of_ender`.
  - Craftable End Portal Frame recipe (obsidian + stone bricks + nether quartz).
  - Basic librarian trade sells ender pearls for emeralds.
- Added focused unit coverage:
  - portal activation requires all eyes and fills the expected 3×3 interior
  - crafting recipes for Eye of Ender + End Portal Frame
  - core item stack sizes and command parsing for ender items

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 10.62s)

### Stage 9 — End: ender pearl throw + dimension-safe respawn

- Ender pearls now have baseline gameplay:
  - right-clicking with an ender pearl throws it as a projectile
  - on impact, the player teleports to a nearby safe spot and takes vanilla-ish self-damage (5.0)
- Respawn is now dimension-safe:
  - dying in the Nether/End respawns you back in the Overworld at the player spawn point (no respawn anchors yet)
  - respawn force-loads chunks and snaps to a safe spawn position
- Added unit coverage for `ProjectileType::EnderPearl` properties and creation.

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 10.61s)

### Stage 9 — End: boss loop (dragon fireballs + deterministic replay)

- Added a boss-ranged attack:
  - new `ProjectileType::DragonFireball` with deterministic physics and damage
  - the Ender Dragon now fires deterministic dragon fireballs when the player is in detection range but outside melee range (faster cadence when enraged).
- Added a shared helper on the world mob type:
  - `Mob::try_spawn_dragon_fireball(...) -> Option<Projectile>` centralizes the deterministic “should fire this tick?” decision.
- Game-layer integration:
  - dragon fireballs are spawned with an in-flight cap (max 4 per dimension)
  - dragon fireballs now damage the player on hit (armor applies) and can kill with a specific death message.
- Added a deterministic replay harness (fixture-driven) for the boss loop:
  - `crates/world/tests/end_boss_replay_snapshot.rs` replays `crates/world/tests/fixtures/end_boss_replay_script.json`
  - golden snapshot: `crates/world/tests/snapshots/micro_end_boss_replay_script.json`

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (includes `micro_end_boss_replay_script_snapshot` and `micro_end_boss_loop_snapshot`)

### Stage 8 — Nether portals (activation + travel)

- Added a `nether_portal` block (non-solid) and wired it into block properties/registry.
- Added `flint_and_steel` as a non-stackable core item:
  - craftable (vanilla-ish iron+flint recipe; mirrored accepted)
  - command name support (`/give flint_and_steel`)
  - droppable + persisted via dropped-item mappings
- Implemented portal activation + travel:
  - right-clicking an obsidian 4×5 frame interior with flint-and-steel fills the 2×3 interior with portal blocks (axis-aware)
  - standing in portal blocks charges for a short delay then triggers Overworld↔Nether travel (cooldown prevents instant bounce)
  - if no destination portal is nearby after travel, one is created deterministically near the arrival point (with basic side-clearance so you can step out)

### Verification

- `cargo fmt --all -- --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 11.71s)

### Stage 6 — Structures: worldgen chest loot uses usable core item IDs

- Fixed structure chest loot generation to emit the legacy core item ID space (e.g., coal=8, iron=7, diamond=14) rather than the experimental `item_ids::*` table, so looted items are usable in crafting/equipment.
- Replaced unsupported ender pearls with arrows.
- Updated the content-pack override worldgen chest loot test accordingly.

### Stage 7 — Mobs: stable mob IDs (persistence-safe)

- Added `Mob.id: u64` (serde-defaulted) and a `GameWorld` allocator to assign IDs on spawn/load.
- Uses the stable ID for villager trading lookup instead of vec indices, avoiding mismatches after despawns/retain.

### Stage 7 — Villagers: deterministic trading UI (professions-lite)

- Right-clicking a villager opens a "Trading" overlay with a small set of deterministic offers.
- Offers are derived from `(world_seed, villager_id)` for replayable determinism; no XP/restock yet.
- Trades use emeralds as a currency (`ItemType::Item(17)`), and outputs spill to the world if inventory is full.

### Stage 7 — Items: bone + emerald end-to-end pickup/drop

- Wired `DroppedItemType::Bone` ↔ core `ItemType::Item(16)` conversion so skeleton bone drops can be picked up and used.
- Added `DroppedItemType::Emerald` and mapped it ↔ core `ItemType::Item(17)` so emeralds can be dropped/spilled/picked up.

### Verification

- `cargo test -p mdminecraft-world --test determinism_worldtest -- --nocapture` (12.68s)
- `cargo test -p mdminecraft-world item_type_from_id_roundtrips`
- `cargo test -p mdminecraft emerald_converts_between_dropped_and_core_item_ids`

### Stage 8 (early) — Active-dimension chunk streaming + `/dimension` travel

- Added `active_dimension` + `spawn_point_dimension` to `GameWorld` and plumbed them through persistence:
  - player save now records the active dimension
  - chunk load/save uses `RegionStore::{load,save}_chunk_in_dimension(...)`
  - block-entity keys are created in the active dimension (so chests/furnaces/etc. don’t collide cross-dimension)
- Added `/dimension <overworld|nether|end>` (and `/dim`) as a debug-friendly travel command:
  - switching dimensions saves/unloads current chunks, resets sims, reloads destination chunks, and snaps the player to a safe nearby location
  - Overworld↔Nether travel applies vanilla-ish x/z scaling (÷8 into Nether, ×8 back out)

### Stage 8 (early) — Nether terrain profile (deterministic)

- Added `TerrainGenerator::generate_chunk_in_dimension(...)` and implemented a minimal deterministic Nether chunk profile:
  - “netherrack” surrogate: `nether_wart_block` as the base filler block (reuses existing item/drop wiring)
  - deterministic cave carving + low-Y lava pockets
  - deterministic quartz-ore and soul-sand scatter in the main band
  - bedrock roof + floor

### Stage 8 (early) — Dimension-scoped entities (mobs/items/projectiles)

- Added `dimension: DimensionId` to dropped items and made `ItemManager` APIs dimension-aware (`spawn_item`, `update`, `pickup_items`, `take_one_near(_if)`, `merge_nearby_items`).
- Added `dimension: DimensionId` to projectiles and made `ProjectileManager` APIs dimension-aware (`spawn`, `update`, `check_hit`); game projectile collision/AoE/pickup logic now operates only in `active_dimension`.
- Added `dimension: DimensionId` to mobs; game mob update/interaction/rendering logic now operates only in `active_dimension`, and all spawn paths set `mob.dimension` (hostile spawns, chunk spawns, village villagers, `/summon`, debug spawns).
- Fixed dispensers/droppers ticking to run in the active dimension (instead of hardcoded Overworld), so Nether ticking is unblocked once portals/content lands.

### Notes / Known gaps

- Portals are still not implemented; travel remains via `/dimension`.
- XP orbs are not yet dimension-scoped (they remain a cross-dimension entity edge case).

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 11.47s)

### Stage 6 — Structures: data-driven worldgen chest loot overrides

- Extended `content_packs/*/loot.json` to support a new `worldgen_chests` section that defines weighted loot tables per structure class (`generic`, `dungeon`, `mineshaft`, `ruin`, `village`).
- Wired `GameWorld::populate_worldgen_chest_loot(...)` to prefer pack-provided tables when present, while keeping the existing hardcoded defaults as the fallback.
- Added regression coverage:
  - `content_pack_loot` test proving chest table overrides respect manifest priority and disabled packs.
  - `game` test proving the override table is actually used when populating a worldgen chest.
...
### Stage 6 — Structures: ASCII template placement (palette scaffolding)

- Added a small reusable `place_ascii_volume_in_chunk(...)` helper with Y-rotation support for authoring tiny structure “templates” without hardcoded nested loops (`crates/world/src/structure_template.rs`).
- Switched the village well’s floor/walls/water and roof placement to use the ASCII template helper, keeping the existing foundation fill + interior clearing + corner posts behavior intact (`crates/world/src/village.rs`).
...
### Stage 7 (early) — Villagers: deterministic village population spawns

- Added deterministic village villager spawn points (2 house spawns + 1 near the well) and exposed them as `mdminecraft_world::village_villager_spawns_for_chunk(...)`.
- Wired the game chunk-generation path to spawn those villagers only when a chunk is generated for the first time (avoids duplication on revisits/saveload).
- Added unit coverage in `mdminecraft-world` to assert expected villager spawn points for a known seed/region.

### Stage 7 (early) — Spawn rules: hostile spawns respect light + solid ground

- Refined hostile mob spawning to be closer to vanilla rules:
  - uses deterministic integer offsets around the player (no trig)
  - requires a solid ground block + 2 blocks of headroom
  - uses a light-level gate (`<= 7`) based on block light and time-scaled skylight (torches prevent spawns)
- Fixed hostile spawn Y placement to spawn at `ground_y + 1.0` (bottom-of-mob), consistent with passive mob spawning.
...
### Stage 6 — Structures: ruin terrain adaptation (foundation fill)

- Added a deterministic foundation-fill pass for surface ruins so they don’t float:
  - for each floor column, fills cobblestone from the first solid ground block up to `base_y - 1`
  - runs before ruin carving/wall placement and does not consume RNG

### Verification

- `cargo fmt --all`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo test --workspace` (notably `tests/determinism_worldtest.rs` finished in 11.42s)
